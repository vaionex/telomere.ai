...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    const findings = derived(() => store_get($$store_subs ??= {}, "$reportsByCategory", reportsByCategory)["nutrition"] || []);
    head("1r68mym", $$renderer2, ($$renderer3) => {
      $$renderer3.title(($$renderer4) => {
        $$renderer4.push(`<title>Nutrigenomics Report — Telomere AI</title>`);
      });
    });
    $$renderer2.push(`<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-24"><div class="mb-8"><a href="/dashboard" class="text-text-tertiary hover:text-text-secondary text-sm mb-4 inline-block">← Back to Dashboard</a> <div class="flex items-center gap-3 mb-2"><span class="text-3xl">🥗</span> <h1 class="text-3xl font-bold">Nutrigenomics</h1></div> <p class="text-text-secondary">How your genes affect caffeine metabolism, lactose tolerance, vitamin needs, folate processing, alcohol sensitivity, and dietary requirements.</p></div> `);
    if (!store_get($$store_subs ??= {}, "$isLoaded", isLoaded)) {
      ...
      ReportSection($$renderer2, {
        title: "Nutrigenomic Findings",
        description: "Variants affecting nutrient metabolism and dietary needs",
        icon: "🥗",
        snps: findings()
      });
    }
